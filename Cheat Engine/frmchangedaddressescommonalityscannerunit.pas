...
uses cefuncproc, ProcessHandlerUnit, frmstructurecompareunit, NewKernelHandler;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        c:=pcontext(owner.group[i][j].context)^; //todo: <
        if regnr=16 then //not a context based value
          values[i][j]:=owner.group[i][j].base
        else
          values[i][j]:=pptruint(ptruint(@c)+contextoffset)^;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RAX{$else}EAX{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RAX{$else}EAX{$endif}) then registers.rax:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RBX{$else}EBX{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RBX{$else}EBX{$endif}) then registers.rbx:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RCX{$else}Ecx{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RCX{$else}Ecx{$endif}) then registers.rcx:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RDX{$else}Edx{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RDX{$else}Edx{$endif}) then registers.rdx:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RSI{$else}Esi{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RSI{$else}Esi{$endif}) then registers.rsi:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RDI{$else}Edi{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RDI{$else}Edi{$endif}) then registers.rdi:=true;
        if (pcontext(group[1][i].context)^.{$ifdef cpu64}RBP{$else}Ebp{$endif}=pcontext(group[2][j].context)^.{$ifdef cpu64}RBP{$else}Ebp{$endif}) then registers.rbp:=true;
        {$ifdef cpu64}
        if (pcontext(group[1][i].context)^.R8=pcontext(group[2][j].context)^.r8) then registers.r8:=true;
        if (pcontext(group[1][i].context)^.R9=pcontext(group[2][j].context)^.r9) then registers.r9:=true;
        if (pcontext(group[1][i].context)^.R10=pcontext(group[2][j].context)^.r10) then registers.r10:=true;
        if (pcontext(group[1][i].context)^.R11=pcontext(group[2][j].context)^.r11) then registers.r11:=true;
        if (pcontext(group[1][i].context)^.R12=pcontext(group[2][j].context)^.r12) then registers.r12:=true;
        if (pcontext(group[1][i].context)^.R13=pcontext(group[2][j].context)^.r13) then registers.r13:=true;
        if (pcontext(group[1][i].context)^.R14=pcontext(group[2][j].context)^.r14) then registers.r14:=true;
        if (pcontext(group[1][i].context)^.R15=pcontext(group[2][j].context)^.r15) then registers.r15:=true;
        {$endif}
        if group[1][i].base=group[2][j].base then registers.base:=true;
      end;
    end;
